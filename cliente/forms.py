...
class DClienteForm(forms.ModelForm):
    class Meta:
        model = Dcliente
        fields = ['cli_razaosocial',     'cli_nomefantasia',     'cli_cnpj',     'cli_im',     'cli_ie',     'cli_email1',     'cli_email2',     'cli_telefone1',     'cli_telefone2',     'cli_endereco',     'cli_complementoendereco',     'cli_cep',     'cli_bairro',     'cli_cidade',     'cli_estado',     'cli_taxacompra',     'cli_multa',     'cli_juros',     'cli_taxarecompra',     'cli_valortotaloperacao',     'cli_datacadastro', 'cli_datafundacao']
...
class DpessoasForm(forms.ModelForm):
    class Meta:
        model = Dpessoas
        fields = ['pes_id', 'pes_nome','pes_cpf','pes_rg','pes_estadocivil','pes_nacionalidade','pes_email1','pes_email2','pes_telefone1','pes_telefone2','pes_profissao','pes_endereco','pes_complementoendereco','pes_cep','pes_bairro','pes_cidade','pes_estado','cli_id','fac_id','pes_tipopessoa','pes_datacadastro', 'pes_datanascimento']
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class DcontratomaeForm(forms.ModelForm):
    class Meta:
        model = Dcontratomae
        fields = ['cma_id', 'cma_id2', 'cli_id', 'fac_id', 'cma_validadecontrato','cma_datacadastro']
    def __init__(self, *args, **kwargs):
        super().__init__(*args, **kwargs)
...
class DcarteiraForm(forms.ModelForm):
    ...
    def clean_car_titulonumero(self):
        ...
        if not Doperacao.objects.filter(
            ...
            ope_numtitulo2=car_titulonumero
        ).exists() and not Doperacao.objects.filter(
            ope_numtitulo3=car_titulonumero
        ).exists() and not Doperacao.objects.filter(
            ope_numtitulo4=car_titulonumero
        ).exists() and not Doperacao.objects.filter(
            ope_numtitulo5=car_titulonumero
        ).exists() and not Doperacao.objects.filter(
